...
class Dog {
   public:
   Dog() { ... }
   Dog(int a) { Dog(); doOtherThings(a); }  // reuse code, but only work in java
};
...
/* Cons:
 * 1. Cumbersome(heavy) code.
 * 2. init() could be invoked by other functions.
 */

// C++ 11:
class Dog {
   int age = 9; // age will initialize every constructor
   public:
   Dog() { ... }
   Dog(int a) : Dog() { doOtherThings(); }// invoked before 2nd constructor start
};
...
// C++ 11
constexpr int A() { return 3; }  // always return a constant
...
int y = cubed(1789);  // computed at compile time, fast bcuz not consume run time
